...
C와 D는 같은 기능을 양쪽에서 나눠 갖기 때문에 **API 계약이 곧 인터페이스**다.
...
### 브랜치 전략
**`develop` 통합 브랜치는 두지 않는다. 기능 브랜치에서 곧바로 `main`으로 PR한다.**

- `main`: 통합 + 배포 기준 브랜치. **직접 push 금지**(브랜치 보호 설정), 반영은 PR로만
...
`main`이 통합 지점과 배포 기준을 겸하므로 **CI 통과를 절대 조건으로 지킨다.** 완충 역할을 하던
`develop`이 없어서, 깨진 상태가 머지되면 그 순간 전원의 기준 브랜치가 깨진다. 실패한 CI를
"나중에 고치겠다"로 넘기고 머지하지 않는다.
...
### PR 규칙
- **PR 대상 브랜치는 항상 `main`이다.**
- PR 제목에 모듈 태그 포함: `[collector] 뉴스 API 클라이언트 구현`
- 본인 모듈 범위를 벗어난 변경이 diff에 섞여 있으면 반려
- 머지 전 최소 1인 리뷰
- 머지 후 기능 브랜치는 삭제한다. 다른 사람이 이어서 작업할 때는 `main`에서 새로 딴다
  (오래 살아있는 브랜치는 `main`과 벌어져 충돌 비용이 커진다).
...
16. **태그 어휘는 하나로 고정한다 — `slug`는 기계 키, `name`은 화면 표시명.** `tags`는 수집기(A)와 피드(C)가 함께 쓰는 마스터 데이터인데, 양쪽이 각자 만들어 어휘가 갈려 있었다. A는 Free News API topic(영문 63개)으로만 태깅하고 개발 시드는 한국어 태그를 따로 만들어서, **한국어 태그를 고른 사용자는 기사가 한 건도 안 잡히는 빈 피드**가 됐다(슬러그까지 충돌해 공존도 불가능했다). 정식 어휘는 Alembic 리비전 `0002_seed_tags`가 소유한다.
    - `slug`: 변하지 않는 기계 키. Free News API topic을 소문자·하이픈으로 정규화한 값(`internet security` → `internet-security`). **수집기는 이 값으로 태그를 찾는다.**
    - `name`: 한국어 표시명. 프런트 필터 칩과 설정 화면이 쓴다. 바꿔도 기사-태그 연결이 끊기지 않는다.
...
19. **배치 스케줄러는 APScheduler로 시작한다 (잠정).** `feature/feed/module-skeleton`의 구현을 현재 스켈레톤에 맞춰 가져왔다. §8 "배치 실행 기술 선정"의 최종 결론이 아니라, **갈아탈 수 있는 형태로 붙인 것**이다 — 스케줄 시각은 설정(`batch_slots`)에서 읽고 배치 로직은 각 모듈의 `run()`을 호출할 뿐이라, 다른 실행기로 바꾸면 `batch/scheduler.py` 하나만 교체된다 (§2 준수).
    - **기본은 꺼져 있다** (`SCHEDULER_ENABLED=false`). 켜면 앱 기동만으로 보관 배치가 도는데 그건 되돌릴 수 없는 삭제다. 개발 중 사고를 막기 위해 명시적으로 켜야 한다.
    - **중복 실행 방지**: APScheduler는 프로세스 안에서 도므로 uvicorn 워커가 둘이면 같은 슬롯이 두 번 실행된다. Redis 락(`lock:job:{job_type}:{slot}:{date}`)이 1차 방어, `batch_jobs.task_ref` UNIQUE가 최종 보증이다.
    - 배치는 서비스 함수가 아니라 `app/batch/{curate,retention}.py`의 `run()`을 호출한다 — 그래야 실행 이력과 오류가 `batch_jobs`/`job_logs`에 남는다 (§9).
    - A의 수집 트리거 엔드포인트는 아직 없다. `COLLECT_TRIGGER_URL`이 비어 있으면 그 단계를 건너뛴다 — 없는 주소로 매 슬롯 실패 로그를 쌓지 않기 위함이다.

### 남은 미결 사항
- **배치 실행 기술 최종 확정** — §8-19로 APScheduler를 **잠정** 채택했다. 단일 프로세스 전제라 서버가 여러 대가 되면 맞지 않는다(cron이나 브로커 기반으로 옮겨야 한다). 배포 형태가 정해지면 다시 본다. 그때까지 §2 규칙(실행기 비의존)을 유지한다. (전원 합의 사항)
