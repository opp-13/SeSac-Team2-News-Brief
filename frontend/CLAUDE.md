...
| **번역** | **전제가 바뀌었다.** 수집 파이프라인이 실제로 번역을 만든다(Google Translate). 서버가 노출 언어 기준으로 `summary.content`에 번역을 담아 보낸다 | 프론트가 할 일은 없다 — 필드 형태가 같아 그대로 그리면 된다. **원문/번역 토글 UI는 여전히 만들지 않는다**(디자인에 없음). 필요해지면 계약부터 넓힌다 |
