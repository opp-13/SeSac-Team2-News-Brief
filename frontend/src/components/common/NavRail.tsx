...
// docs/figma-export/components/NavRail.tsx 이식. §2 표: route/navigate prop을
// useLocation/useNavigate로, isAdmin prop을 useAuth()로 대체(관리자 메뉴는
// useAuth().isAdmin일 때만 노출). 원본에서도 안 쓰이던 isLoggedIn prop은 제거.
...
export default function NavRail() {
  ...
  const isActive = (path: string) => {
    if (path === '/admin/pipeline') return pathname.startsWith('/admin')
